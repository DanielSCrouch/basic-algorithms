...
class BinaryTree(object):
  ...
  def breadth_first_traversal(self) -> Iterator:
    queue = [self.root]
    while len(queue) > 0:
      node = queue[0] 
      queue = queue[1:]

      # Add child nodes in left to right order
      if node.left is not None:
  ...
  def depth_first_traversal(self, order: Union['preorder', 'inorder', 'postorder']) -> Iterator:

    if order not in ('preorder', 'inorder', 'postorder'):
      raise ValueError(f"ordering type {order} should be one of 'preorder', 'inorder', 'postorder'")

    def preorder(node: Node) -> Node:
      yield node
      if node.left:
        for child in preorder(node.left):
          yield child
      if node.right:
        for child in preorder(node.right):
          yield child

    def inorder(node: Node) -> Node:
      if node.left:
        for next_node in inorder(node.left):
          yield next_node
      yield node
      if node.right:
        for next_node in inorder(node.right):
          yield next_node
      
    def postorder(node: Node) -> Node: 
    ...
    for child in traversal_map[order](self.root):
      yield child.value
